...
std::string toLower(const std::string& str) {
    ...
    std::transform(lower.begin(), lower.end(), lower.begin(), toLowerChar/*[](unsigned char c) { return std::tolower(c); }); "[] == lambda"*/);
    return lower;
}

bool load_bad_words(const std::string& filename) {
    std::ifstream file(filename.c_str());
    if (!file) {
        std::cerr << "Erreur : impossible d’ouvrir le fichier de bad words : " << filename << std::endl;
        return false;
    }
    ...
    while (std::getline(file, word)) {
        if (!word.empty())
            bad_words.push_back(toLower(word));
    }
...
bool contains_bad_word(const std::string& message) {
    std::string msg_lower = toLower(message);
    for (size_t i = 0; i < bad_words.size(); ++i) {
        if (msg_lower.find(bad_words[i]) != std::string::npos)
            return true;
    }
    return false;
}
...
void handle_command(const std::string &line)
{
    ...
    if (cmd[0] == ':')
    {
        ...
        else
        {
            ...
            else if (contains_bad_word(rest.c_str() + rest.find(':') + 1))//old_else if(check_for_bad_words(rest.c_str() + rest.find(':') + 1))
            {
                cmd = cmd.c_str() + 1;
...
int main(int argc, char **argv)
{
    ...
    if (!load_bad_words("badwords.txt"))
        return 1;
